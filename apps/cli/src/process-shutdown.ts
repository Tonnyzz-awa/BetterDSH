...
export function createProcessShutdown(
  ...
  const completeOnce = (code: number): void => {
    ...
    completed = true
    clearExitTimeout()
    // If a signal already set an exitCode, keep the more specific reason; only
    // publish a natural completion code when nothing has started terminating yet.
    if (process.exitCode === undefined) complete(code)
  }
