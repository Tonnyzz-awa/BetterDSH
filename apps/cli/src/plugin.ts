 ...
 */
function readInstalledBundleManifest(packageName: string, profileDir: string): { dir?: string; manifest?: ProfileManifest; error?: Error } {
  try {
...
function exportsPatch(packageName: string, profileDir: string): { isBundle: boolean; reason?: string } {
  const resolved = readInstalledBundleManifest(packageName, profileDir)
  if (resolved.error !== undefined) {
    return { isBundle: false, reason: resolved.error.message }
  }
  const patch = resolved.manifest?.dsh?.bundle?.patch
  if (patch === undefined) {
    return { isBundle: false, reason: `${packageName} does not declare dsh.bundle in its package.json` }
  }
...
function reconcilePlugins(before: ProfileManifest, profileDir: string): void {
  ...
  let changed = false
  const installed = new Map<string, { isBundle: boolean; reason?: string }>()
  for (const packageName of dependencies) {
    const resolved = exportsPatch(packageName, profileDir)
    installed.set(packageName, resolved)
    if (resolved.isBundle && !plugins.includes(packageName)) {
      plugins.push(packageName)
      changed = true
    } else if (!resolved.isBundle && !beforeDeps.has(packageName)) {
      warnings.push(`${NAME}: warning: ${packageName} is installed but will not load as a profile layer${resolved.reason === undefined ? '' : ` (${resolved.reason})`}`)
    }
  }
  ...
  for (const packageName of [...plugins]) {
    ...
    const resolved = installed.get(packageName) ?? exportsPatch(packageName, profileDir)
    if (wasDependency && !resolved.isBundle) {
      plugins.splice(plugins.indexOf(packageName), 1)
      changed = true
      warnings.push(`${NAME}: warning: removed ${packageName} from dsh.profile.bundles${resolved.reason === undefined ? '' : ` (${resolved.reason})`}`)
    }
  }
