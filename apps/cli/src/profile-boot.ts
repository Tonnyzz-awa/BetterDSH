...
export async function runProfile(options: RunProfileOptions): Promise<{ ctx: Context; shutdown: ProcessShutdown }> {
  ...
  // Keep explicit handlers so the boot path can remove them on settle; otherwise
  // repeated runProfile() calls in the same process accumulate listeners.
  const onSigterm = () => { interrupt(0) }
  ...
  if (!signalShutdown.signal.aborted
    ...
    let profileWatchStopped: (() => void) | undefined
    let homeWatchStopped: (() => void) | undefined
    try {
      ...
      profileWatchStopped = await watchUserPatches(ctx, {
        binName: NAME,
        filename: composed.profile.patchPath,
        compose: composeLive,
      })
      homeWatchStopped = await watchUserPatches(ctx, {
        binName: NAME,
