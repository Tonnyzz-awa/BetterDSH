...
describe('shipped agent presets gate both shell tools by platform', () => {
  ...
  it('minimal gates its bash PTY stack off win32 and mounts tool-pwsh there instead', () => {
    const entries: unknown = yaml.load(
    ...
    const byId = new Map(entries.flatMap(entry => (
      typeof entry === 'object' && entry !== null && typeof (entry as Record<string, unknown>).id === 'string'
        ? [[(entry as Record<string, unknown>).id as string, entry as Record<string, unknown>]]
        : []
    )))
    // The persistent bash PTY stack is a group gated off win32: its
    // dsh-terminal-bash backend spawns /bin/bash and has no Windows shell.
    const ptyStack = byId.get('persistent-shell')
    if (ptyStack === undefined) throw new TypeError('minimal preset must mount the persistent-shell group')
    const ptyDisabled = ptyStack.disabled
    expect(ptyDisabled).toMatchObject({ __jsExpr: expect.any(String) as string })
    const ptyExpr = (ptyDisabled as { __jsExpr: string }).__jsExpr
    expect(Boolean(evaluate({ process: { platform: 'win32' } }, ptyExpr)), 'PTY stack on win32').toBe(true)
    expect(Boolean(evaluate({ process: { platform: 'linux' } }, ptyExpr)), 'PTY stack on linux').toBe(false)
    // win32 sessions mount the one-shot pwsh tool instead; POSIX gates it off.
    const pwsh = byId.get('tool-pwsh')
    if (pwsh === undefined) throw new TypeError('minimal preset must mount tool-pwsh for win32')
    expect(pwsh.disabled).toMatchObject({ __jsExpr: expect.any(String) as string })
    const pwshExpr = (pwsh.disabled as { __jsExpr: string }).__jsExpr
    expect(Boolean(evaluate({ process: { platform: 'win32' } }, pwshExpr)), 'tool-pwsh on win32').toBe(false)
    expect(Boolean(evaluate({ process: { platform: 'linux' } }, pwshExpr)), 'tool-pwsh on linux').toBe(true)
    // minimal never mounts the host bash tool row on any platform.
    expect(byId.has('tool-bash'), 'tool-bash must be absent from minimal').toBe(false)
  })
})
