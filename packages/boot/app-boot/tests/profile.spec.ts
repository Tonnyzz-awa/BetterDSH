...
describe('loadProfile', () => {
  ...
  it('writes manifests back without changing the round-tripped value', () => {
    const dir = tmp()
    const manifest = {
      name: 'dsh-profile-roundtrip',
      private: true,
      dependencies: {},
      dsh: { profile: { bundles: ['@deepseek-ai/dsh-base'] } },
    }
    writeProfileManifest(dir, manifest)
    expect(readProfileManifest('t', dir)).toEqual(manifest)
  })
...
describe('healProfilesModuleFallback', () => {
  it('links the app and bundle dependency surface flat under profiles/node_modules', () => {
    const anchor = stageInstallation({
      'bundle-a': { patch: '[]\n', deps: { 'dep-of-a': '0.0.0', 'ghost-dep': '0.0.0', 'optional-of-a': '0.0.0' } },
      'plain-lib': {},
      'optional-of-a': {},
    })
    ...
    for (const name of ['bundle-a', 'plain-lib', 'dep-of-a', 'optional-of-a', 'dsh-app']) {
