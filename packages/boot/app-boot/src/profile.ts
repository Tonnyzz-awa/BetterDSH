...
export function initProfile(dir: string, bundles: readonly string[]): void {
  ...
  if (!existsSync(manifestPath)) {
    ...
    writeAtomicText(manifestPath, JSON.stringify(manifest, undefined, 2) + '\n')
  }
  const patchPath = join(dir, PROFILE_PATCH_FILENAME)
  if (!existsSync(patchPath)) writeAtomicText(patchPath, PROFILE_PATCH_TEMPLATE)
  const workspacePath = join(dir, 'pnpm-workspace.yaml')
  if (!existsSync(workspacePath)) writeAtomicText(workspacePath, PROFILE_PNPM_WORKSPACE)
}
...
function ensureSymlink(link: string, target: string): void {
  ...
  try {
    ...
    if (errno !== 'EEXIST' && process.platform === 'win32') {
      try {
...
export function healProfilesModuleFallback(installAnchor: string, home: string = resolveDshHome()): void {
  ...
  for (let next = queue.shift(); next !== undefined; next = queue.shift()) {
    ...
    for (const dep of [...Object.keys(next.manifest.dependencies ?? {}), ...Object.keys(next.manifest.optionalDependencies ?? {}), ...Object.keys(next.manifest.peerDependencies ?? {})]) {
...
export function writeProfileManifest(dir: string, manifest: ProfileManifest): void {
  writeAtomicText(join(dir, 'package.json'), JSON.stringify(manifest, undefined, 2) + '\n')
}
...
/**
 * Normalize one exact installation-owned shipped tuple back to the published
 * template while preserving every other manifest field.
 *
 * This is intentionally a write-back: only the exact shipped tuple is
 * canonicalized, so the on-disk manifest stays aligned with the installation's
 * own default profile definition. Any other bundle list is user-owned and is
 * returned untouched.
 */
